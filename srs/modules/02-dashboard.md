...
| FR-M02-15 | Thẻ **"Số dư hiện có"** (nguồn `FinanceService::summary()`): số dư hiện tại (cho phép âm, cảnh báo khi âm), tổng đã nạp (góp vốn) và tổng đã chi; liên kết tới `finance.overview`. |
...
| Service | `app/Services/AttendanceCloser.php`, `app/Services/FinanceService.php` (thẻ số dư) |
