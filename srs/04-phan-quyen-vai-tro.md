...
| Quản lý tài chính (quỹ, thu/chi, góp vốn, công nợ) | ✅ | ❌ |
| Quản trị lương (tạo kỳ, tính, duyệt, chi, thêm khoản tay) | ✅ | ❌ |
| Xem phiếu lương của **chính mình** (`/luong/cua-toi`) | ✅ | ✅ (khi kỳ đã duyệt/đã chi) |
...
| Đăng nhập | `/`, `/tai-khoan`, `/cham-cong*`, `/nghi-phep*`, `/kpi*`, `/nhan-vien` (index/show), `/luong/cua-toi`, `/luong/{period}/phieu/{payslip}` | `auth` |
| Quản trị (ghi) | `nhan-vien` create/store/edit/update/destroy; `nghi-phep` status/destroy; `kpi` create/store/edit/update/destroy; toàn bộ `cai-dat*`; toàn bộ `tai-chinh*` (`finance.*`); toàn bộ `luong*` quản trị (`payroll.periods.*`, `payroll.payslips.items.*`) | `auth` + `can:admin` |
| Quản trị (trong controller) | `kpi/giai-doan*` (theo phụ trách); `payroll.payslips.show` (admin hoặc chủ phiếu, kỳ đã duyệt/đã chi) | `auth` + kiểm tra thủ công |
...
| BR-SEC-08 | User chỉ xem được **phiếu lương của chính mình** và chỉ khi kỳ ở trạng thái `approved`/`paid`; xem phiếu người khác hoặc kỳ chưa duyệt → HTTP 403. |
